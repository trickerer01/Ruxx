...
from .gui_defines import SLASH, UNDERSCORE
from .tagger import append_filtered_tags
from .tagsdb import load_tag_aliases
from .utils import normalize_path, trim_underscores
...
def untag_files(files: Iterable[str]) -> int:
    ...
    try:
        ...
        for full_path in files:
            ...
            if fname_match := re_media_tagged_name.fullmatch(full_name):
...
def retag_files(files: Sequence[str], re_tags_to_process: re.Pattern, re_tags_to_exclude: re.Pattern) -> int:
    ...
    try:
        ...
        base_path: str = os.path.split(normalize_path(files[0], False))[0]
        ...
        for full_path in files:
            ...
            if fname_match := re_media_untagged_name.fullmatch(full_name) if tags else None:
